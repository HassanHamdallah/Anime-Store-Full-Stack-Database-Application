...
def search_products():
    ...
    try:
        ...
        min_rating = request.args.get('minRating')  # NEW: Rating filter
        sort_by = request.args.get('sortBy', 'name')  # name, price_asc, price_desc, rating

        # ADVANCED: Dynamic query building with multiple conditions including rating
        query = """
            SELECT 
                p.productId,
                p.name,
                ...
                CASE 
                    ...
                    ELSE 'Out of Stock'
                END as stockStatus,
                COALESCE((SELECT AVG(r.rating) FROM Rating r WHERE r.productId = p.productId), 0) as averageRating,
                (SELECT COUNT(*) FROM Rating r WHERE r.productId = p.productId) as totalRatings
            FROM Product p
            LEFT JOIN Category c ON p.categoryId = c.categoryId
            WHERE 1=1
        """
        ...
        # Rating filter - filter products with average rating >= minRating
        if min_rating:
            query += """ AND COALESCE((SELECT AVG(r.rating) FROM Rating r WHERE r.productId = p.productId), 0) >= %s"""
        ...
        for p in products:
            p['unitPrice'] = float(p['unitPrice']) if p['unitPrice'] else 0
            p['totalStock'] = int(p['totalStock']) if p['totalStock'] else 0
            p['averageRating'] = round(float(p['averageRating']), 1) if p['averageRating'] else 0
            p['totalRatings'] = int(p['totalRatings']) if p['totalRatings'] else 0
...
# ============================================
# NORMAL QUERY: Get all products (for order editing)
# ============================================
@customer_products_bp.route('/api/products/all', methods=['GET'])
...
# ============================================
# NORMAL QUERY: Get product stock
# ============================================
@customer_products_bp.route('/api/products/<int:product_id>/stock', methods=['GET'])
def get_product_stock(product_id):
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("""
            SELECT COALESCE(SUM(ib.quantityOnHand), 0) as stock
            FROM InventoryBalance ib
            WHERE ib.productId = %s
        """, (product_id,))
        result = cursor.fetchone()
        return jsonify({'stock': int(result['stock']) if result else 0})
    except Error as e:
        return jsonify({'error': str(e)}), 500
    finally:
        if conn.is_connected():

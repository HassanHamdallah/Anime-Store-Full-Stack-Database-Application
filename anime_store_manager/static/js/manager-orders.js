...
function filterOrders() {
    ...
    if (currentFilters.status !== 'all') {
        filtered = filtered.filter(o => {
            // Handle "Paid" filter to match both "Paid" and "New Order (Paid)"
            if (currentFilters.status === 'Paid') {
                return o.status === 'Paid' || o.status === 'New Order (Paid)';
            }
            return o.status === currentFilters.status;
        });
    }
...
function calculateStats() {
    ...
    const totalRevenue = managerOrdersData
        .filter(o => o.status !== 'Cancelled')
        .reduce((sum, o) => sum + parseFloat(o.totalAmount || 0), 0);
    const pendingOrders = managerOrdersData.filter(o => o.status === 'Paid' || o.status === 'New Order (Paid)' || o.status === 'Processing').length;
...
// Sync allocations - clean up orphaned allocations for removed items
async function syncAllocations() {
    if (!currentOrderId) return;
    
    if (!confirm('This will remove allocations for products that are no longer in the order and restore inventory to warehouses. Continue?')) return;
    
    try {
        const res = await fetch(`/api/manager/orders/${currentOrderId}/sync-allocations`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        ...
        if (res.ok) {
            if (result.cleaned > 0) {
                showNotification(`Cleaned ${result.cleaned} orphaned allocation(s)`);
                // Refresh the modal
                openFulfillmentModal(currentOrderId);
            } else {
                showNotification('No orphaned allocations found');
            }
        } else {
            showNotification(result.error || 'Failed to sync allocations', 'error');
        }
    } catch (e) {
        console.error('Sync allocations error:', e);
...
function loadWarehousesForProduct() {
    ...
    if (productData && productData.warehouses) {
        productData.warehouses.forEach(wh => {
            // Filter: Manager sees all, Employee only sees assigned warehouses
            if (userRole === 'Manager' || wh.managerStaffId === userId) {

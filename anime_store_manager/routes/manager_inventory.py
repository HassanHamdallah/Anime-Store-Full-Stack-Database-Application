...
def create_warehouse():
    ...
    try:
        cursor = conn.cursor()
        manager_staff_id = data.get('managerStaffId') if data.get('managerStaffId') else None
        cursor.execute("INSERT INTO Warehouse (name, location, managerStaffId) VALUES (%s, %s, %s)", 
                      (data.get('name'), data.get('location'), manager_staff_id))
        conn.commit()
...
def update_warehouse(id):
    ...
    try:
        cursor = conn.cursor()
        manager_staff_id = data.get('managerStaffId') if data.get('managerStaffId') else None
        cursor.execute("UPDATE Warehouse SET name = %s, location = %s, managerStaffId = %s WHERE warehouseId = %s",
                      (data.get('name'), data.get('location'), manager_staff_id, id))
        conn.commit()
...
@manager_inventory_bp.route('/api/manager/warehouses/<int:id>', methods=['GET'])
